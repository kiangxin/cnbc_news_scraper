...
def cnbc_scrap(urls, driver, start, end,thread_index):
    ...
    logging.basicConfig(filename=f'logs_{date_str}.log', level=logging.ERROR, format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    if not check_internet_connection():
        ...
        for url in urls [start:end]:
            try:
                ...
                try:
                    make_it = driver.find_element(By.CLASS_NAME, "MakeItGlobalNav-styles-makeit-logo--sXqSs")
                ...
                if "select" in url:
                    try:
                        author_element = driver.find_element(By.XPATH,"//div[@class='Author-styles-select-authorNameAndSocial--C51G7']")
                        html = author_element.get_attribute('innerHTML')
                        soup = BeautifulSoup(html, 'html.parser')
                       
                        author_name = soup.find('a', {'class': 'Author-styles-select-authorName--IoDuN'}).text
                    except NoSuchElementException:
                        logging.error(f"{url} - No Author found")
                        author_name = "No Author Name Available"
                        pass
                    try:
                        headline = driver.find_element(By.CLASS_NAME, "ArticleHeader-styles-select-headline--n2eyV").text
                    except NoSuchElementException:
                        headline ="No Headline Available"
                        logging.error(f"{url} - No Headline found")
                        pass
                    try:
                        category = driver.find_element(By.CLASS_NAME, "ArticleHeader-styles-select-eyebrow--Yjj64").text
                    except NoSuchElementException:
                        logging.error(f"{url} - No Category found")
                        category="No Category Available"
                        pass
                    try:
                        time_div = driver.find_element(By.XPATH,"//div[@class='ArticleHeader-styles-select-time--dEL7X']")
                    ...
                    try:
                        author_element = driver.find_element(By.CLASS_NAME,"Author-styles-makeit-authorName--_ANaL")
                        author_soup = BeautifulSoup(author_element.get_attribute('innerHTML'), 'html.parser')
                        ...
                        pass
                    try:
                        headline = driver.find_element(By.CLASS_NAME, "ArticleHeader-styles-makeit-headline--l_iUX").text
                    except NoSuchElementException:
                        headline ="No Headline Available"
                        logging.error(f"{url} - No Headline found")
                        pass
                    try:
                        category = driver.find_element(By.CLASS_NAME, "ArticleHeader-styles-makeit-eyebrow--Degp4").text
                    except NoSuchElementException:
                        logging.error(f"{url} - No Category found")
                        category="No Category Available"
                        pass
                    ...
                    try:
                        author_element = driver.find_element(By.CLASS_NAME,"Author-authorName")
                        author_soup = BeautifulSoup(author_element.get_attribute('innerHTML'), 'html.parser')
                        author_name = author_soup.text.strip()
                    except NoSuchElementException:
                        try:
                            img_elem = driver.find_element(By.XPATH,'//div[@class="Author-author"]//img')
                            ...
                            pass
                    try:
                        headline = driver.find_element(By.CLASS_NAME, "ArticleHeader-headline").text
                    except NoSuchElementException:
                        headline ="No Headline Available"
                        logging.error(f"{url} - No Headline found")
                        pass
                    ...
                    try:
                        ...
                        pass
                try:
                    group_div = wait.until(EC.presence_of_all_elements_located((By.XPATH, "//div[@class='group']")))
                    paragraphs = []
                    full_content = ''
                ...
                #Key points/summary
                try:
                    key_points_div = driver.find_element(By.CLASS_NAME,'RenderKeyPoints-list')
        ...
        while True:
            try:
                ...
                with open(csv_filename, 'a', newline='', encoding='utf-8') as csv_file:
                    fieldnames = ['URL', 'Author', 'Headline', 'Category', 'Published Time', 'Content', 'Summary']
                    ...
                    writer.writerow(scraped_item)

            except queue.Empty:
                # If the queue is empty, sleep for a while to avoid busy-waiting
                time.sleep(1)
    
    csv_thread = Thread(target=write_to_csv)
    csv_thread.start()

def check_internet_connection(url='http://www.google.com', timeout=10, retry_interval=20):
    while True:
        try:
...
def main():
    urls =["https://www.cnbc.com/2023/10/14/these-top-colleges-promise-no-student-loan-debt.html",
